...
const testCases = [
  ...
  },
  {
    ...
    question:
      "Please create a new file called test-claude-output.txt with 'Hello from Claude CLI test'",
    isEditRequest: true,
  ...
  },
  {
    ...
    question:
      'This is a very long question that should test how the system handles longer inputs. '.repeat(
        10
      ),
    isEditRequest: false,
    options: { workingDirectory: process.cwd() },
...
async function testClaudeExecution(testCase) {
  return new Promise((resolve) => {
    ...
    console.log(
      `[CLAUDE CLI] Input: ${fullInput.length} chars, ${needsPermissions ? 'edit' : 'read-only'} mode, ${timeout / 1000}s timeout`
    );
    ...
    claudeProcess.on('close', (code) => {
      ...
      console.log(
        `[CLAUDE CLI] Exit code: ${code}, stdout: ${stdout.length} chars, stderr: ${stderr.length} chars`
      );
