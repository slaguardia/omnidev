...
import { useInvalidateRalphTasks } from '@/hooks/queries/useRalphTasks';
...
import { DEFAULT_WORKFLOW_DEFINITION, findStageDefinition } from '@/lib/workflow/definition';
import {
  prefetchRalphBoardBootstrap,
  useRalphBoardBootstrap,
} from '@/hooks/queries/useRalphBoardBootstrap';
...
export default function RalphBoardTab({ isPaletteOpen }: { isPaletteOpen?: boolean }) {
  ...
  } = useRalphBoardBootstrap({ archived: showArchived });

  const handleShowArchivedChange = useCallback(
    (value: boolean) => {
      if (value) {
        void prefetchRalphBoardBootstrap(queryClient, { archived: true });
      }
      setShowArchived(value);
    },
    [queryClient, setShowArchived]
  );
  const tasks = bootstrapData?.tasks ?? [];
  const projects = bootstrapData?.projects ?? [];
  const playbooks = bootstrapData?.playbooks ?? [];
  const definition = bootstrapData?.definition ?? DEFAULT_WORKFLOW_DEFINITION;
  const taskStates = getTaskStates(definition);
  ...
  useEffect(() => {
    ...
    const handlePaletteToggleArchived = () => {
  ...
  }, [handleExpandTask, queryClient, setShowArchived]);
  ...
  return (
    <div className="space-y-6">
      ...
      {isBootstrapError && (
        <div
          role="alert"
          className="rounded-lg border border-danger-200 bg-danger-50 px-3 py-2 text-sm text-danger-800 dark:border-danger-800/60 dark:bg-danger-950/40 dark:text-danger-200"
        >
          Could not load board data.{' '}
          {bootstrapError instanceof Error ? bootstrapError.message : 'Request failed.'}
        </div>
      )}
      ...
      {/* Board list view — bootstrap uses placeholderData so the shell never waits on a spinner */}
      <ListView
        definition={definition}
        tasks={filteredTasks}
