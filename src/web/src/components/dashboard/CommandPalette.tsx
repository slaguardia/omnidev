...
export default function CommandPalette({ isOpen, onClose }: CommandPaletteProps) {
  ...
  // Only fetch when open — otherwise these duplicate Ralph board bootstrap + /api/ralph/tasks on every dashboard load
  const { data: tasks = [] } = useRalphTasks({ enabled: isOpen });
  const { data: workspaces = [] } = useRalphWorkspaces({ includeBranches: false, enabled: isOpen });
  const { data: projects = [] } = useRalphProjects({ enabled: isOpen });
