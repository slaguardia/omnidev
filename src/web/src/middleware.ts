...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    // Do not call `/api/auth/session-health` from here. Middleware runs in a restricted
    // runtime where same-origin `fetch` to the app often fails (e.g. `Error: fetch failed`
    // from Next's sandbox `context.fetch` on Railway), which spams logs and adds no value.
    // Stale JWTs vs user store are handled by NextAuth `session` / `jwt` callbacks (they
    // call `getUser()` / `isJwtUserStillValid`). Route handlers and RSC still see an empty
    // session when the store no longer matches.
