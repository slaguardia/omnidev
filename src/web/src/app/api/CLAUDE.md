...
| Endpoint            | Method | Purpose                                 | Auth Required |
| ------------------- | ------ | --------------------------------------- | ------------- |
| `/api/jobs/[jobId]` | GET    | Legacy job status/result (`data/jobs/`) | Yes           |

### Ralph tasks & V2 (database jobs; requires `pnpm worker`)

See root `CLAUDE.md` for **`/api/ralph/tasks/...`** (including **`POST .../run-stage`**) and **`/api/v2/tasks`**, **`/api/v2/jobs`**. These are **not** served by the legacy queue worker in `instrumentation.ts`.
...
| Endpoint                      | Method | Purpose                      | Auth Required |
| ----------------------------- | ------ | ---------------------------- | ------------- |
| `/api/config/validate`        | POST   | Validate configuration       | Yes           |
| `/api/config/test-connection` | POST   | Test Git remote connectivity | Yes           |
