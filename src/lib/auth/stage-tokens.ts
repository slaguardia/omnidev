 ...
 */
export function generateStageToken(options: {
  jobId: string;
  taskId: string;
  permissions: CliPermission[];
  ttlMs?: number;
 ...
 */
export function validateStageToken(rawToken: string): StageTokenInfo | null {
  const tokenHash = hashToken(rawToken);
  const row = dbGetStageTokenByHash(tokenHash);
 ...
 */
export function revokeJobTokens(jobId: string): number {
 ...
 */
export function revokeExpiredTokens(): number {
