...
import { switchBranch as gitSwitchBranch } from '@/lib/git/branches';
import { pullChanges as gitPullChanges } from '@/lib/git/remotes';
import { setWorkspaceGitConfig as gitSetConfig, getWorkspaceGitConfig as gitGetConfig, unsetWorkspaceGitConfig as gitUnsetConfig } from '@/lib/git/config';
import { type GitCloneOptions } from '@/lib/git/types';
import * as WorkspaceManagerFunctions from '@/lib/managers/WorkspaceManager';
...
export async function cloneRepository(
  ...
  try {
    ...
    const size = await calculateWorkspaceSize(workspacePath);
 ...
 */
// getGitOperations function removed - use standalone git functions instead

/**
 * Calculate total size of workspace directory
 */
async function calculateWorkspaceSize(directoryPath: FilePath): Promise<number> {
  // Simple estimation - return 0 for now since we removed caching
  // Could implement actual directory size calculation if needed
  return 0;
}
