...
async function workspaceGitRoot(workspace: Workspace): Promise<FilePath> {
  return resolveWorkspaceGitRoot(workspace);
}
...
export async function switchBranch(
  ...
  try {
    ...
    const root = await workspaceGitRoot(workspace);
    const switchResult = await gitOps.switchBranch(root, branchName);
...
export async function pullChanges(workspaceId: WorkspaceId): Promise<AsyncResult<void>> {
  try {
    ...
    const gitOps = new GitOperations();
    const root = await workspaceGitRoot(workspace);
    const pullResult = await gitOps.pullChanges(root);
    if (!pullResult.success) {
      return pullResult;
    }

    // Update commit hash in memory and persist to disk
    const commitResult = await gitOps.getCurrentCommitHash(root);
    if (commitResult.success) {
...
export async function cleanupWorkspace(
  ...
  try {
    ...
    // Remove durable workspace directory only (ephemeral workspaces have no path)
    if (workspace.path) {
      try {
      ...
      if (workspace.repoUrl === repoUrl) {
        ...
        if (workspace.targetBranch === targetBranch) {
          ...
          try {
            const gitOps = new GitOperations();
            const root = await workspaceGitRoot(workspace);
            const currentCommitResult = await gitOps.getCurrentCommitHash(root);
            if (
...
export async function setWorkspaceGitConfig(
  ...
  try {
    ...
    const gitOps = new GitOperations();
    const root = await workspaceGitRoot(workspace);
    const setConfigResult = await gitOps.setWorkspaceGitConfig(root, gitConfig);
...
> {
  try {
    ...
    const gitOps = new GitOperations();
    const root = await workspaceGitRoot(workspace);
    const getConfigResult = await gitOps.getWorkspaceGitConfig(root);
...
export async function unsetWorkspaceGitConfig(
  ...
  try {
    ...
    const gitOps = new GitOperations();
    const root = await workspaceGitRoot(workspace);
    const unsetConfigResult = await gitOps.unsetWorkspaceGitConfig(root, keys);
...
export async function initializeGitWorkflow(
  ...
  try {
    ...
    if (!targetBranch) {
      ...
      const defaultBranchResult = await gitOps.getDefaultBranch(root);
    ...
    const prepareResult = await prepareWorkspaceForEdit(root, targetBranch);
    ...
    if (effectiveSourceBranch !== targetBranch) {
      ...
      const switchResult = await gitOps.switchBranch(root, effectiveSourceBranch);
      if (!switchResult.success) {
        return {
          success: false,
          error: new Error(
