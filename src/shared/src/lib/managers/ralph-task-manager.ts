...
export async function initializeRalphTaskManager(): Promise<AsyncResult<void>> {
  ...
  try {
    ...
    const entries = await dbGetAllIndexEntries();
...
export async function createRalphTask(
  ...
  try {
    ...
    const taskNumber = await dbNextTaskNumber();
    const taskData = {
      ...input,
      id: generateTaskId(),
      taskNumber,
...
export async function updateRalphTask(
  ...
  try {
    const existingTask = await dbGetTask(taskId);
    if (!existingTask) {
      return {
        success: false,
        error: new Error(`Task ${taskId} not found`),
...
export async function transitionRalphTask(
  ...
  try {
    ...
    if (preloadedTask) {
      ...
      const loaded = await dbGetTask(taskId);
      if (!loaded) {
        return {
          success: false,
          error: new Error(`Task ${taskId} not found`),
...
export async function getAllRalphTasks(): Promise<AsyncResult<RalphTaskIndexEntry[]>> {
  try {
    const tasks = await dbGetAllIndexEntries();
...
export async function deleteRalphTasksByWorkspace(
  ...
  try {
    const deletedCount = await dbDeleteByWorkspace(workspaceId);
...
export async function getChildRalphTasks(
  ...
  try {
    const children = await dbGetChildTasks(parentId);
    const entries: RalphTaskIndexEntry[] = await Promise.all(
      children.map(async (task) => ({
        ...
        childCount: await dbGetChildCount(task.id),
...
export async function updateRalphProject(
  ...
  try {
    const updated = await dbUpdateProject(id, updates);
    if (!updated) {
...
export async function deleteRalphProject(id: string): Promise<AsyncResult<void>> {
  try {
    const deleted = await dbDeleteProject(id);
    if (!deleted) {
      return { success: false, error: new Error('Project not found') };
    }
...
export async function createRalphPlaybook(input: {
  ...
  try {
    ...
    await dbCreatePlaybook(playbook);
...
export async function listRalphPlaybooks(): Promise<AsyncResult<RalphPlaybook[]>> {
  try {
    const playbooks = await dbListPlaybooks();
...
export async function getRalphPlaybook(id: string): Promise<AsyncResult<RalphPlaybook>> {
  try {
    const playbook = await dbGetPlaybook(id);
    if (!playbook) {
...
export async function updateRalphPlaybook(
  ...
  try {
    ...
    const updated = await dbUpdatePlaybook(id, dbUpdates);
    if (!updated) {
      return { success: false, error: new Error('Playbook not found') };
    }
    const playbook = await dbGetPlaybook(id);
...
export async function deleteRalphPlaybook(id: string): Promise<AsyncResult<void>> {
  try {
    const deleted = await dbDeletePlaybook(id);
    if (!deleted) {
      return { success: false, error: new Error('Playbook not found') };
    }
 ...
 */
export async function getNextPlaybookStage(
  taskId: string,
  currentStage: string
): Promise<{ nextStage: string | null; playbook: RalphPlaybook | null }> {
  const task = await dbGetTask(taskId);
  if (!task) return { nextStage: null, playbook: null };

  if (!task.playbookId) return { nextStage: null, playbook: null };

  const dbPb = await dbGetPlaybook(task.playbookId);
 ...
 */
export async function validatePlaybookTransition(
  task: RalphTask,
  toStatus: string
